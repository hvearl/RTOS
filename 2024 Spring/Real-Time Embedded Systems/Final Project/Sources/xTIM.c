...
const uint16_t volumeLookuptTable[]={
...
void tim3Config(void){
    ...
    TIM3->CR2 &= ~TIM_CR2_MMS;    // Select master mode
    TIM3->CR2 |= TIM_CR2_MMS_2;    // 100 = OC1REF as TRGO
    
    TIM3->DIER |= TIM_DIER_TIE;    // Trigger interrupt enable
    TIM3->DIER |= TIM_DIER_UIE;    // Update interrupt enable
    
    TIM3->CCMR1 &= ~TIM_CCMR1_OC1M;
    TIM3->CCMR1 |= (TIM_CCMR1_OC1M_1 | TIM_CCMR1_OC1M_2);  // 0110 = PWM mode 1
    
    TIM3->PSC = 15;            // 16 MHz / (7+1) = 2 MHz timer ticks
    TIM3->ARR = 999;      // 2 MHz / (70+1) = 28.169 kHz interrupt rate; 64 entry look-up table = 440.14 Hz sine wave
    TIM3->CCR1 = 999;        // 50% duty cycle
    TIM3->CCER |= TIM_CCER_CC1E;
    TIM3->CR1 |= TIM_CR1_CEN;    // Enable timer  
	NVIC_EnableIRQ(TIM3_IRQn); // this makes the interupt actually get called
}

void TIM3_IRQHandler(void){
	//poll distance sensor, throw into queue
